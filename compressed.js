		...
		images: {
			...
			{	// Create image
				...
				{	// Already a image, so copy over values
					...
					image.title = obj.atitle || image.atitle || $(obj).attr('title') || $(image).attr('title');
				...
				{	// We are an element
					...
					if ( obj.attr('src') || obj.attr('href') )
					{
						...
						image.title = obj.atitle || image.atitle || obj.attr('title') || obj.attr('alt') || $(image).attr('title');
...
/*				prev:		'images/prev.gif',
				...
				prev:		'/nav/icons/48/action-1left.png',
				next:		'/nav/icons/48/action-1right.png',
				blank:		'/dastyle/nul.png',
		...
		text: {
			...
			of:			'sur',
			close:		'Fermer ×',
			closeInfo:	'Vous pouvez aussi cliquer en-dehors de l\'image pour la fermer.',
			/*download:	'Direct link to download the image.',*/
			help: {
				close:		'Cliquer pour fermer',
				interact:	'Fonctions par survol'
			},
/*
			about: {
...
/*		ie6_upgrade:	true,		// show ie6 upgrade message*/
		...
		domReady: function ( )
		{
			...
			$('body').append('<div id="lightbox-overlay"><div id="lightbox-overlay-text">'+/*(this.show_linkback?'<p><span id="lightbox-overlay-text-about"><a href="#" title="'+this.text.about.title+'">'+this.text.about.text+'</a></span></p><p>&nbsp;</p>':'')+*/'<p><span id="lightbox-overlay-text-close">'+this.text.help.close+'</span><br/>&nbsp;<span id="lightbox-overlay-text-interact">'+this.text.help.interact+'</span></p></div></div><div id="lightbox"><div id="lightbox-imageBox"><div id="lightbox-imageContainer"><img id="lightbox-image" /><div id="lightbox-nav"><a href="#" id="lightbox-nav-btnPrev"></a><a href="#" id="lightbox-nav-btnNext"></a></div><div id="lightbox-loading"><a href="#" id="lightbox-loading-link"><img src="' + this.files.images.loading + '" /></a></div></div></div><div id="lightbox-infoBox"><div id="lightbox-infoContainer"><div id="lightbox-infoHeader"><span id="lightbox-caption"><span id="lightbox-caption-title" href="#" title="' + this.text.download + '"></span><span id="lightbox-caption-seperator"></span><span id="lightbox-caption-description"></span></span></div><div id="lightbox-infoFooter"><span id="lightbox-currentNumber"></span><span id="lightbox-close"><a href="#" id="lightbox-close-button" title="'+this.text.closeInfo+'">' + this.text.close + '</a></span></div><div id="lightbox-infoContainer-clear"></div></div></div></div>');
			...
			$('#lightbox-nav-btnPrev').unbind().hover(function() { // over
				...
				$(this).css({ 'background' : 'url(/nav/icons/48/action-1left.png) left 45% no-repeat' });
			},function() { // out
			...
			$('#lightbox-nav-btnNext').unbind().hover(function() { // over
				...
				$(this).css({ 'background' : 'url(/nav/icons/48/action-1right.png) right 45% no-repeat' });
			},function() { // out
				$(this).css({ 'background' : 'transparent url(' + $.Lightbox.files.images.blank + ') no-repeat' });
			}).click(function() {
...
/*					$('#lightbox-caption-title').html(image.title || 'Untitled').attr('href', image.src);*/
					$('#lightbox-caption-title').html($(image).attr('title') || '&nbsp;').attr('href', image.src);
...
/*					 });*/
					...
					if ( !this.images.last(image) ) {
						// Not first, show button$(this).prop('title')
